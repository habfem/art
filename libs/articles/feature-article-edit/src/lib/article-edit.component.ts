...
export class ArticleEditComponent implements OnInit, OnDestroy {
  ...
  submit() {
    this.data$.pipe(take(1)).subscribe((data: { tagList: string }) => {
      const tagList = data.tagList.split(',').map(tag => tag.trim());
      this.store.dispatch(articleEditActions.publishArticle({ tagList }));
    });
  }
